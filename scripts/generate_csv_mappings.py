...
def create_csv_graph() -> Graph:
    csv = pd.read_csv("/home/agar2/Documents/1Projects/7WorkProjects/2Projects/ERA/ttl-terms-export-v2.csv")
    ...
    for index, entity in zip(range(0,len(csv)), csv.Entity):
        graph.add((Namespace.term(csv_ns, str(index)), RDFS.label, Literal(entity)))
    return graph
